...
let sample n corpus_filename max_length init =
  failwith @@ Format.sprintf "unimplemented: %d %s --sample %d --initial-words %s"
    n corpus_filename max_length (
      match init with
      | None -> ""
      | Some words -> String.concat ~sep:" " words
    )

let most_frequent n corpus_filename k =
  failwith @@ Format.sprintf "unimplemented: %d %s --most-frequent %d"
    n corpus_filename k
